...
func createDtabaseIfNotExists(client *azcosmos.Client, dbName string) error {
	...
	if err != nil {
		...
		if errors.As(err, &respErr) {
			...
			case 409:
				logging.Logger.Info().Msg("Database already exists")
			case 201:
				logging.Logger.Info().Msg("Database created")
			default:
				logging.Logger.Err(err).Msg("Failed to create database")
				return err
			}
...
func createContainerIfNotExists(client *azcosmos.DatabaseClient, dbName, containerName string) error {
	...
	if err != nil {
		...
		if errors.As(err, &respErr) {
			switch respErr.StatusCode {
			case 409:
				logging.Logger.Info().Msg("Container already exists")
			case 201:
				logging.Logger.Info().Msg("Container created")
			default:
				logging.Logger.Err(err).Msg("Failed to create container")
				return err
			}
